  ...
  },
  {
    ...
    steps: {
      fr: [
        ...
        "Bon à savoir : les réglages voisins\nDans la même zone, vous trouverez « Turn Copilot Dashboard on or off » (interrupteur global), « Copilot Dashboard Auto Enablement » (activation automatique pour les nouveaux managers éligibles) et « Copilot Dashboard delegation ». Un simple accès + auto-enablement suffit généralement pour démarrer.",
        "Vérifier l'organigramme et la hiérarchie d'équipes\nUne fois l'accès donné, ouvrez le dashboard et vérifiez que la structure d'organisation (managers, équipes) s'affiche correctement. C'est ce qui permet la lecture par équipe et par niveau hiérarchique. Si la hiérarchie est absente ou fausse, c'est le signal qu'il faut fournir des données d'organisation.",
        "Si besoin : (re)configurer les données d'organisation\nQuand l'organigramme natif ne suffit pas, importez vos données organisationnelles (rattachements managers, entités, sites) pour fiabiliser la segmentation. C'est l'étape « avancée » à ne déclencher que si l'étape 4 révèle un manque.",
        "Aller plus loin : rapports personnalisés & Power BI\nSur la base du Copilot Dashboard, vous pouvez ensuite bâtir des rapports personnalisés et des tableaux Power BI (Copilot Analytics) pour des vues sur-mesure, croisées avec vos propres dimensions métier. À réserver aux besoins de pilotage avancés, une fois le socle « sur étagère » en place.",
      ],
      en: [
        ...
        "Good to know: the neighbouring settings\nIn the same area you'll find “Turn Copilot Dashboard on or off” (global switch), “Copilot Dashboard Auto Enablement” (auto-enable for newly eligible managers) and “Copilot Dashboard delegation”. Simple access + auto-enablement is usually enough to get started.",
        "Check the org chart and team hierarchy\nOnce access is granted, open the dashboard and confirm the organizational structure (managers, teams) displays correctly. That's what powers the per-team and per-level reading. If the hierarchy is missing or wrong, it's the signal to provide organizational data.",
        "If needed: (re)configure organizational data\nWhen the native org chart isn't enough, upload your organizational data (manager relationships, entities, sites) to make segmentation reliable. This is the “advanced” step to trigger only if step 4 reveals a gap.",
        "Going further: custom reports & Power BI\nBuilding on the Copilot Dashboard, you can then create custom reports and Power BI dashboards (Copilot Analytics) for tailored views crossed with your own business dimensions. Keep this for advanced steering needs, once the out-of-the-box baseline is in place.",
      ],
    },
    stepImages: [
      null,
    ...
    imageWidth: "full",
    info: {
      fr: "Commencez « sur étagère » : donnez l'accès, activez l'auto-enablement, et regardez ce que le dashboard remonte. N'engagez l'import de données d'organisation ou les rapports Power BI que si un manque concret apparaît. Réf. : Learn « Copilot Dashboard » (org-team-insights/copilot-dashboard), « Manage settings » (advanced/admin/manage-settings-copilot-dashboard) et « Copilot Analytics / Power BI ».",
      en: "Start “out of the box”: grant access, enable auto-enablement, and see what the dashboard surfaces. Only trigger org-data upload or Power BI reports if a concrete gap appears. Refs: Learn “Copilot Dashboard” (org-team-insights/copilot-dashboard), “Manage settings” (advanced/admin/manage-settings-copilot-dashboard) and “Copilot Analytics / Power BI”.",
    },
